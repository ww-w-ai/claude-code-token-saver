 ...
 *   ~/.claude/cc-token-saver-data/{projectHash}/{sessionId}/compact.txt
...
// Cache:  ~/.claude/cc-token-saver-data/{projectHash}/{sessionId}/compact.txt
function deriveCachePath(jsonlAbsPath) {
  ...
  const cacheDir = path.join(os.homedir(), ".claude", "cc-token-saver-data", projectHash, sessionId);
...
async function main() {
  ...
  const projectCacheDir = path.join(os.homedir(), ".claude", "cc-token-saver-data", projectHash);
  ...
  for await (const line of rl) {
    ...
    if (type === "user") {
      ...
      if (cmdNameMatch) {
        const cmdName = cmdNameMatch[1].trim(); // e.g. "/clear", "/cc-token-saver:continue"
      ...
      if (inContinueSkill && Array.isArray(content)) {
        for (const block of content) {
          if (block && block.type === "tool_use" && block.name === "Read" && block.input && block.input.file_path) {
            const m = block.input.file_path.match(/\/cc-token-saver-data\/[^/]+\/([^/]+)\/compact/);
