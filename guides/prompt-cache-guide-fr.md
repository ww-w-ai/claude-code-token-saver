...
**C'est exactement pourquoi le plugin cc-token-saver affiche un avertissement d'expiration de cache apres 1 heure d'inactivite.** Lorsque l'avertissement apparait, verifiez la taille de votre contexte actuel :
...
Le plugin cc-token-saver est concu pour automatiser ou simplifier toutes ces strategies.
...
cc-token-saver detecte les etats d'inactivite de 1 heure et **affiche automatiquement un avertissement**. Lorsque l'avertissement apparait, utiliser la methode 1 ci-dessus (`/clear` + `/continue`) pour continuer dans une nouvelle session est l'approche la plus economique.
...
cc-token-saver guide automatiquement ce modele de separation du travail au demarrage de la session.
...
`/setup-git-lite` de cc-token-saver reduit les instructions Git par defaut de Claude Code (~2 200 tokens) a un noyau de 280 tokens — une reduction d'environ 88% du prompt systeme lie a Git par session.
...
cc-token-saver injecte automatiquement ce guide de selection d'outils a l'IA au demarrage de la session, encourageant l'utilisation d'outils legers en priorite.
