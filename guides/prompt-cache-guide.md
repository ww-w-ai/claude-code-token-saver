...
**This is exactly why the cc-token-saver plugin displays a cache expiry warning after 1 hour of inactivity.** When the warning appears, check your current context size:

- **Small context**: Cache re-creation cost is manageable. Just continue working — the cost is low.
- **Large context**: Cache cost will be significant. We recommend `/clear` followed by `/continue last` to resume in a new session. The continue skill automatically restores your previous conversation context, so your workflow isn't interrupted.

## Strategies to Reduce Cache Costs

The cc-token-saver plugin is designed to automate or simplify all of these strategies.

### 1. Keep Context Small — `/clear` + `/continue` ⭐
...
When the topic changes or the conversation gets long, run `/clear` to reset, then `/continue last` to restore previous context. `/continue` restores previous conversations without any LLM calls, so the cost is zero.
...
cc-token-saver detects 1-hour idle states and **automatically displays a warning**. When the warning appears, using method 1 above (`/clear` + `/continue`) to continue in a new session is the most economical approach.
...
cc-token-saver automatically guides this work-separation pattern at session start.
...
cc-token-saver's `/setup-git-lite` reduces Claude Code's default Git instructions (~2,200 tokens) to a core 280 tokens — an approximately 88% reduction in Git-related system prompt per session.
...
cc-token-saver automatically injects this tool selection guide to the AI at session start, encouraging the use of lightweight tools first.
