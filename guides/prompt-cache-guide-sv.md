...
**Det ar precis darfor cc-token-saver-pluginen visar en cache-utgångsvarning efter 1 timmes inaktivitet.** Nar varningen visas, kontrollera din nuvarande kontextstorlek:
...
cc-token-saver-pluginen ar utformad for att automatisera eller forenkla alla dessa strategier.
...
cc-token-saver upptacker 1 timmes inaktivitet och **visar automatiskt en varning**. Nar varningen visas ar det mest ekonomiska tillvagagangssattet att anvanda metod 1 ovan (`/clear` + `/continue`) for att fortsatta i en ny session.
...
cc-token-saver varleder automatiskt till detta arbetsfordelningsmonster vid sessionsstart.
...
`/setup-git-lite` fran cc-token-saver minskar Claude Codes standardmaossiga Git-instruktioner (~2 200 tokens) till en karna pa 280 tokens — en minskning pa cirka 88 % av Git-relaterad systemprompt per session.
...
cc-token-saver injicerar automatiskt denna verktygsguide till AI:n vid sessionsstart och uppmanar till att anvanda lättviktiga verktyg forst.
