...
| 🔄 /continue | מחליף את /compact — אפס קריאות LLM, אפס עלות, אפס אובדן מידע | שחזור context בחינם |
...
## 🔄 תכונה 3: /continue — שחזור Context
...
`/continue` לוקח גישה שונה לחלוטין. הוא מעבד מראש את transcript הסשן הקודם וטוען אותו ישירות. אין קריאת LLM. אין עלות. השיחה המקורית משוחזרת כפי שהיא.
...
שימוש: `/clear` ואז `/continue`. תראה רשימת סשנים קודמים. בחר אחד לשחזור. לשחזור מהיר: `/continue last`.
...
| צהריים/ישיבה | משך 2 שעות                                 | —                           | —                                  |
| חזרה        | ⚡ Token Guardian חוסם → /clear + /continue | —                           | ＄0 (אין קריאות LLM)               |
...
[הפעלה מחדש של סשן]
    │
    └─ /continue → משחזר context קודם ללא עלות (אין קריאות LLM)
```
...
- **skills/** — איך עובדים /continue ו-/usage-view, תבניות prompt
...
- **משך 1+ שעה?** `/clear` → חזור → `/continue`. Context משוחזר ב-$0.
...
- **עקוב אחרי [CTX] כמו מד מהירות.** אף מגבלת קצב לא תעצור אותך — אבל context ב-500K+ אומר שכל קריאת API עולה 2-3 פעמים יותר ממה שצריך. `/clear` → `/continue` הוא בחינם ומאפס את מכפיל העלויות שלך לקו הבסיס.
